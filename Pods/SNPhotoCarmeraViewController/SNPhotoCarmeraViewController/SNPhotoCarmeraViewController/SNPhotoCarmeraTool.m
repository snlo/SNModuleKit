...
@implementation SNPhotoCarmeraTool
...
+ (void)showAlertStyle:(UIAlertControllerStyle)style title:(NSString *)title msg:(NSString *)message chooseBlock:(void (^)(NSInteger actionIndx))block  actionsStatement:(NSString *)cancelString, ... NS_REQUIRES_NIL_TERMINATION {
		...
		if (argsArray.count > 1) {
			if (isSettingColor) {
				if (styleAction == UIAlertActionStyleCancel) {
					[action setValue:[SNPhotoCarmeraTool sharedManager].contentColor forKey:@"titleTextColor"];
				} else {
					[action setValue:[SNPhotoCarmeraTool sharedManager].blackColor forKey:@"titleTextColor"];
				}
			}
		}
	...
	alertController.view.tintColor = [SNPhotoCarmeraTool sharedManager].blackColor;
